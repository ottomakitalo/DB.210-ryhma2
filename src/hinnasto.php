...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['paivita_hinnasto']) 
    ...
    if ($xml === false) {
        echo "XML-tiedoston lukeminen epäonnistui.";
        exit;
    }

    // Hae toimittajan nimi XML:stä
    $toimittaja = (string)$xml->toimittaja->toim_nimi;

    if ($toimittaja === '') {
        echo "Toimittajaa ei löytynyt XML-tiedostosta.";
        exit;
    }

    // Valmistele tarvikkeet XML:stä
    $uudet_tarvikkeet = [];
    ...
    if (count($uudet_tarvikkeet) === 0) {
        echo "XML:stä ei löytynyt yhtään tarviketta.";
        exit;
    }

    if (!pg_query($yhteys, 'BEGIN')) {
        echo "Tietokantavirhe: transaktion aloittaminen epäonnistui.";
        exit;
    }

    $ok = true;

    // Hae nykyiset tarvikkeet kyseiseltä toimittajalta
    $res = pg_query_params($yhteys, 'SELECT * FROM tarvike WHERE toimittaja = $1', array($toimittaja));
    if ($res === false) {
        pg_query($yhteys, 'ROLLBACK');
        echo "Tietokantavirhe: tarvikkeiden hakeminen epäonnistui.";
        exit;
    }
    ...
    // Päivitä tai lisää tarvikkeet XML:n perusteella
    ...
    if ($ok) {
        ...
        pg_query($yhteys, 'ROLLBACK');
        echo "Päivitys epäonnistui. Tarkista XML tiedoston rakenne.";
        exit();
    }
}
?>
